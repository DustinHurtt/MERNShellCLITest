...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        // init()o;
        (0, init_1.init)();
        if (config_1.mernshellConfig.displayHelp) {
        ...
        yield config_1.mernshellConfig.init();
        // mernshellConfig.debug();
        if (config_1.mernshellConfig.isOutOfSync) {
        ...
        if (config_1.mernshellConfig.devMode) {
            config_1.mernshellConfig.debug();
        }
        const newInDirPath = cmd_1.FolderOps.inDirectory();
        const outDirPath = cmd_1.FolderOps.outDirectory(config_1.mernshellConfig.name);
        // return;
        const vars = { name: config_1.mernshellConfig.name };
        const templatedFiles = yield (0, create_template_folder_1.createTemplateFolder)({
            inDir: newInDirPath,
            outDir: outDirPath,
            vars,
        }, {
            dryRun: config_1.mernshellConfig.dryRun,
        });
