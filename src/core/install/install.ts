...
export async function install(basePath: string, fileNames: string[]) {
  const { name: outDir } = mernshellConfig;
...
export class MakeInstall {
  static async install(basePath: string) {
    ...
    const { template } = mernshellConfig;
